...
async def run(query: str, *, live: bool, use_cache: bool) -> None:
    ...
    # Both "official" and "local" 様式 docx are filled in place: leaf-aware
    # OfficialFormFiller preserves every paragraph (instructions, table
    # captions, blank cells) and only inserts generated body text under
    # each leaf-level heading. 参考様式 / 記載項目 docs are treated the same
    # way — the publisher's structure IS the format we must follow.
